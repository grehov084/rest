...
*/
        ...
        function agregateIBlock($arElems, $iblockId){
            if(CModule::IncludeModule("iblock")){
                ...
                foreach($arIdsForRemove as $arId){
                    ...
                    $res = $el->Update($arId, $arLoadProductArray);
                }
                
                foreach($arElems as $key=>$arElem){
                    foreach($arExistElems as $key=>$arExistElem){
                        if($arElem["ID"] == $arExistElem["PROPERTIES"]["CRM_ID"]["VALUE"]){
                            ...
                            $allowAdding = true;
                            $edit = false;
                            if($arElem["CODE"] == $arExistElem["CODE"]){
                                $allowAdding = false;
                                $edit = true;
                                $idForEdit = $arExistElem["ID"];
                                break;
                    ...
                    if($allowAdding == true){
                        ...
                        //ресайз изображений
                        $arLoadProductArray = Array( 
                            "IBLOCK_SECTION_ID" => $iblockSectionId,          // элемент лежит в корне раздела
                            "IBLOCK_ID"      => $iblockId,
                        ...
                        if($PRODUCT_ID = $el->Add($arLoadProductArray))
                            AddMessage2Log("New ID: ".$PRODUCT_ID);
                        else
                    ...
                    elseif($edit == true){
                        ...
                        $arLoadProductArray = Array( 
                            ...
                            "NAME"           => $arElem["NAME"],
                            "CODE"           => $arElem["CODE"],
                            "ACTIVE"         => "Y", //написать изменение активности
                            "PREVIEW_PICTURE" => CFile::MakeFileArray($arElem["PROPERTIES"]["PHOTO"]["VALUE"]),           
                            "DETAIL_PICTURE" => CFile::MakeFileArray($arElem["PROPERTIES"]["PHOTO"]["VALUE"]),
                            "LICENSE_POPUP" => "Y",
                        );
                        $res = $el->Update($idForEdit, $arLoadProductArray);
                        //AddMessage2Log($res);
                    }
                    else{
                        continue;
        ...
        function getElem($iblockCrmId, $crmIblockSectionId){
            $result = CRest::call(
                ...
                [
                    ...
                    'IBLOCK_TYPE_ID' => 'lists',
                    'FILTER' => array(
                        "PROPERTY_187" => $crmIblockSectionId,
                    )
                ]
            );
            foreach($result['result'] as $key=>$arItem) {
                foreach($arItem as $keyProp => $arField) {
                    if(mb_strpos($keyProp,'PROPERTY_') === 0) {
                        ...
                        if($this->propMap[$keyProp]['TYPE'] == 'F') {
                            ...
                            $file["result"][0] = str_replace("/services/lists/34/file/0/".$arItem["ID"]."/PROPERTY_194/", "", $file["result"][0]);
                            ...
                            $resultFile = file_get_contents($url, false, stream_context_create(array(
                                'http' => array(
                                    'method'  => 'POST',
                        ...
                        /* дополнительно для диллеров получать координаты по известному адресу */
                ...
                $result['result'][$key]['PROPERTIES'] = array();
    ...
    if($_REQUEST["id"] != null){
        ...
        //AddMessage2Log($targetIbId);
        
    }
    if($targetIbId != null){
        $rest = new Rest($iblockCrmId);
        $rest->getElemsList($iblockCrmId,  $filter, $targetIbId, $_REQUEST["id"]);
    }
?>